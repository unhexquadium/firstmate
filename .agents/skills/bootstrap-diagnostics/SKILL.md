...
- `WORKTREE_PROTECTION: task <id>: skipped|failed: <detail>` - the locked post-reboot sweep could not re-establish the durable exclusion or identity-bound presence protection required for that recorded task worktree.
  Do not spawn another task from the same project until the recorded path and Treehouse status are reconciled and a rerun of session start is silent; the task's branch ref may still survive, but another lease of that directory can reset its checkout and discard uncommitted work.
