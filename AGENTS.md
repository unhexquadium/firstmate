...
state/               runtime records and signals; gitignored
  ...
  .worktree-protection/  durable exclusions for quiescent legacy Treehouse task worktrees and identity-bound cwd guards for occupied ones; locked bootstrap registers them and normal teardown retires each task's record after Treehouse returns the worktree
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the seven bootstrap MUTATING sweeps - recorded-worktree protection, non-executing legacy PR-check migration, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, and Relay artifact writes - run only when this session actually holds the lock from step 1; the four network ones among them run in the deferred stage rather than in this section.
