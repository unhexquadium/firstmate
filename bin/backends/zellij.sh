...
# Mirrors tmux's pane_current_path poll used to confirm a leased-worktree cd.
#
...
# whatever directory the pane's shell was in when it launched an interactive
# `treehouse get` as a foreground command - it never follows that command's own internal `cd`
