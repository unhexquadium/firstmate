...
# any error. Mirrors tmux's pane_current_path poll used to confirm the exact
# durable leased-worktree cd.
#
...
# shell `cd`s or enters a subshell. Reading it here would make a future
# foreground-command path transition invisible even though the current durable
# lease flow uses a top-level cd.
# `.result.pane.foreground_cwd` tracks the ACTUALLY RUNNING foreground
# process's cwd instead - confirmed live against both top-level and nested cwd
# transitions.
...
# spawn-time commands (the leased-worktree cd, the GOTMPDIR export). `pane run` types
