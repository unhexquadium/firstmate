...
`current_directory` follows the top-level shell `cd` used by the durable Treehouse lease flow, but not a foreground subshell such as the one opened by legacy interactive `treehouse get` acquisition.
