...
[`docs/architecture.md`](architecture.md#worktrees-not-branches-in-your-checkout) owns the durable lease and post-reboot protection boundary, while `bin/fm-worktree-lease-lib.sh` owns its exact mechanics.
...
A herdr, zellij, or cmux home is therefore never told `tmux` is missing, and the `treehouse` compatibility check requires `get --lease`, `get --lease-holder`, `get --json`, `status --json`, `return --if-lease-holder`, and `return --if-lease-id` only for backends that actually use treehouse.
