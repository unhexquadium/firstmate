...
A herdr, zellij, or cmux home is therefore never told `tmux` is missing, and the `treehouse` compatibility check requires `get --lease`, `get --lease-holder`, `status --json`, and `return --if-lease-holder` only for backends that actually use treehouse.
