...
`pane_cwd` follows the top-level shell `cd` used by the durable Treehouse lease flow, but not a foreground subshell such as the one opened by legacy interactive `treehouse get` acquisition.
Spawn-time confirmation retains the stronger active probe: it sends begin and end markers around `pwd`, captures the marked block, and joins wrapped path lines.
The probe is not advertised as a general live-cwd API.
